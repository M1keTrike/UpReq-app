...
- [ ] T110 [P] Prueba de flujo completo en `integration_test/recording_flow_test.dart` (validaciones V1 y V2): grabar, marcar, detener y reproducir — **dos bugs reales diagnosticados y arreglados esta sesión (2026-08-12), pendiente confirmar el paso completo en dispositivo**:
  ...
  2. **Bug real de la app**, encontrado al arreglar el anterior y llegar más lejos en el flujo: tras "Pasar a En curso", el botón "Grabar entrevista" no aparecía nunca (`canRecord` quedaba en `false` para siempre). Causa: `sessionCaptureProvider` (`lib/features/recordings/presentation/session_capture_provider.dart`) calculaba `canRecord` a partir de `SessionStatusReader.find()` (una lectura puntual) dentro del `asyncMap` de `recordingRepository.watchBySession(id)` — ese stream solo reemite cuando cambia la tabla de grabaciones, nunca cuando cambia el estado de la sesión, así que `canRecord` quedaba congelado en el valor de la primera suscripción (sesión todavía "planeada") mientras la pantalla de detalle siguiera montada, exactamente el escenario de un uso manual normal (crear sesión → ver el detalle → pasar a en curso sin navegar fuera). Arreglado añadiendo `SessionStatusReader.watch(SessionId)` (reactivo, respaldado por `SessionRepository.watchDetail`) y recombinando `sessionCaptureProvider` con `combineLatest2` sobre ambos streams. `dart analyze --fatal-infos`, los 250 tests de `flutter test` (unit+widget) y las cinco puertas automáticas (cobertura 87.85%, import boundaries, no-network-deps, pinned-versions) pasan con el fix
  - **Pendiente**: confirmar en dispositivo físico el resto del flujo (grabar, marcar, detener, reproducir) después del fix — la última corrida automática llegó hasta iniciar la grabación sin colgarse ni fallar antes de interrumpirse por trabajo de diagnóstico; el usuario terminará esa verificación a mano
- [ ] T111 [P] Prueba de flujo en `integration_test/interrupted_recovery_test.dart` (validación V3): simular la interrupción y verificar ambas ramas de recuperación — **no ejecutada todavía**; comparte el mismo helper de creación de sesión que T110 (ya arreglado), así que debería dejar de estar bloqueada, pero falta confirmarlo ejecutándola
- [ ] T112 [P] Prueba de flujo en `integration_test/transcription_flow_test.dart` (validaciones V4 y V6) con `FakeTranscriber`: transcripción pendiente sin modelo, y segmentos con salto de reproducción con modelo — **no ejecutada todavía**, misma situación que T111
- [ ] T113 [P] Prueba de aislamiento en `integration_test/recording_isolation_test.dart` (validación V7): con dos proyectos poblados, ningún listado de grabaciones cruza datos — **no ejecutada todavía**, misma situación que T111
- [ ] T114 [P] Prueba de solo lectura en `integration_test/closed_project_capture_test.dart` (validación V8): con proyecto cerrado, el control de grabar **no aparece en ningún instante**, ni siquiera durante la carga — **no ejecutada todavía** (esta no crea sesión, así que no compartía el bloqueo de T110-T113 en primer lugar)
