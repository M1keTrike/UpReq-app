...
- [X] T110 [P] Prueba de flujo completo en `integration_test/recording_flow_test.dart` (validaciones V1 y V2): grabar, marcar, detener y reproducir
- [X] T111 [P] Prueba de flujo en `integration_test/interrupted_recovery_test.dart` (validación V3): simular la interrupción y verificar ambas ramas de recuperación
- [X] T112 [P] Prueba de flujo en `integration_test/transcription_flow_test.dart` (validaciones V4 y V6) con `FakeTranscriber`: transcripción pendiente sin modelo, y segmentos con salto de reproducción con modelo
- [X] T113 [P] Prueba de aislamiento en `integration_test/recording_isolation_test.dart` (validación V7): con dos proyectos poblados, ningún listado de grabaciones cruza datos
- [X] T114 [P] Prueba de solo lectura en `integration_test/closed_project_capture_test.dart` (validación V8): con proyecto cerrado, el control de grabar **no aparece en ningún instante**, ni siquiera durante la carga
  - **T110–T114, advertencia de verificación**: los cinco archivos existen, pasan `dart analyze` y doblan todo el hardware/red vía `integration_test/support/hardware_fakes.dart` (nuevo) — pero **no se pudieron ejecutar en este entorno**: `flutter test integration_test/` exige un dispositivo/emulador Android o iOS conectado (`flutter devices` no reporta ninguno aquí, solo Windows/Chrome/Edge, no soportados por este proyecto), y esa limitación es anterior a esta fase — los cuatro `integration_test/*.dart` del incremento 1 tampoco corren en este entorno. Corrieron pruebas de análisis estático únicamente. **Ejecutar `flutter test integration_test/` en una máquina con un dispositivo/emulador conectado antes de confiar en ellas por completo**: un hallazgo de este tipo no vale hasta ejecutarlo de verdad. De paso se corrigió un defecto real que esto habría escondido: `integration_test/support/test_app.dart` (`pumpTestApp`) nunca sobreescribía `sessionStatusReaderProvider` como sí hace `main.dart` — cualquier prueba de captura habría lanzado `UnimplementedError` de inmediato
- [X] T115 [P] Documentar en `README.md` los comandos nuevos y el requisito de descargar el modelo antes de transcribir
- [ ] T116 Ejecutar las validaciones manuales V1 a V5 y V7 a V9 del [quickstart.md](quickstart.md) en un dispositivo Android físico — **bloqueado**: este entorno no tiene un dispositivo Android físico ni emulador disponible
- [ ] T117 Ejecutar la validación V6 del [quickstart.md](quickstart.md) con una entrevista real de al menos 5 minutos en español, con dos voces y ruido de fondo, y **registrar las seis mediciones** de la tabla: duración de la pasada definitiva, su relación con la duración del audio, errores por minuto, aporte del glosario, impacto de la pasada en vivo y consumo de batería — **bloqueado**, misma razón que T116
