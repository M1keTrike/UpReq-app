...
- [X] T110 [P] Prueba de flujo completo en `integration_test/recording_flow_test.dart` (validaciones V1 y V2): grabar, marcar, detener y reproducir — **tres bugs reales diagnosticados y arreglados, confirmado en dispositivo físico (Android 16, A059P) el 2026-08-12**:
  1. **Bug del arnés de prueba** (no de la app): el `TODO(device-run 2026-08-12)` de la sesión anterior culpaba al botón "Guardar" del formulario de sesión, pero eso era un diagnóstico erróneo — con dumps del árbol en vivo se confirmó que el botón sí existía. El fallo real era que `SessionDraft.validate()` exige **al menos un participante** (data-model.md) y la prueba nunca creaba ni seleccionaba un interesado antes de guardar; `Guardar` fallaba en silencio (mensaje "La sesión debe tener al menos un participante."), y el siguiente `tester.tap(find.text('Entrevista Uno'))` coincidía sin querer con el `EditableText` del campo Título todavía abierto (Flutter's `find.text` también matchea `EditableText` por contenido del controller), así que la prueba seguía adelante sobre la pantalla equivocada. Arreglado añadiendo la creación de un interesado y marcando su casilla en "Participantes" antes de guardar la sesión
  2. **Bug real de la app**, encontrado al arreglar el anterior y llegar más lejos en el flujo: tras "Pasar a En curso", el botón "Grabar entrevista" no aparecía nunca (`canRecord` quedaba en `false` para siempre). Causa: `sessionCaptureProvider` (`lib/features/recordings/presentation/session_capture_provider.dart`) calculaba `canRecord` a partir de `SessionStatusReader.find()` (una lectura puntual) dentro del `asyncMap` de `recordingRepository.watchBySession(id)` — ese stream solo reemite cuando cambia la tabla de grabaciones, nunca cuando cambia el estado de la sesión, así que `canRecord` quedaba congelado en el valor de la primera suscripción (sesión todavía "planeada") mientras la pantalla de detalle siguiera montada, exactamente el escenario de un uso manual normal (crear sesión → ver el detalle → pasar a en curso sin navegar fuera). Arreglado añadiendo `SessionStatusReader.watch(SessionId)` (reactivo, respaldado por `SessionRepository.watchDetail`) y recombinando `sessionCaptureProvider` con `combineLatest2` sobre ambos streams
  3. **Segundo bug real de la app**, encontrado al ejecutar por primera vez en dispositivo físico (el timing real de Gradle/instalación/stream expuso una carrera que el test en memoria no alcanzaba a disparar): la hoja de recuperación (T066, no descartable sin elegir) aparecía **encima de la grabación activa**, bloqueando los botones de marcar y detener, justo al empezar a grabar. Causa: `FindInterrupted.call()` (T060) promueve *cualquier* grabación en estado `recording` a `interrupted` **escribiendo en la base de datos**, y `sessionCaptureProvider` la invocaba en cada emisión del stream combinado — incluido el tick de `elapsed` cada segundo — así que el segundo después de iniciar una grabación, el propio proceso la encontraba, la interpretaba como huérfana de una ejecución anterior y la marcaba `interrupted` **en la base**, aunque `ActiveCaptureNotifier` la tuviera activa en ese mismo proceso. No era solo un glitch de UI: corrompía el estado persistido de una grabación en curso. Arreglado en `session_capture_provider.dart` para no invocar `findInterrupted()` mientras `active != null` — si este proceso ya posee una captura activa, por definición no hay nada huérfano que buscar
  - `dart analyze --fatal-infos`, las pruebas afectadas de `flutter test` y la prueba de integración en dispositivo físico pasan con los tres fixes
- [X] T111 [P] Prueba de flujo en `integration_test/interrupted_recovery_test.dart` (validación V3): simular la interrupción y verificar ambas ramas de recuperación — **confirmado en dispositivo físico el 2026-08-12**, tras arreglar dos bugs adicionales expuestos al ejecutarla (no presentes en T110, que solo tiene un `testWidgets`): (1) el arnés de sesión sin interesado, igual que T110; (2) `appRouter` (`lib/core/router/app_router.dart`) es un singleton a nivel de módulo que sobrevive entre los dos `testWidgets` del archivo, así que el segundo test arrancaba donde el primero lo había dejado en vez de en la lista de proyectos — arreglado con `appRouter.go('/')` al principio de `pumpTestApp()` (`integration_test/support/test_app.dart`), que beneficia a todo archivo con más de un `testWidgets` (T112 también los tiene)
- [X] T112 [P] Prueba de flujo en `integration_test/transcription_flow_test.dart` (validaciones V4 y V6) con `FakeTranscriber`: transcripción pendiente sin modelo, y segmentos con salto de reproducción con modelo — **confirmado en dispositivo físico el 2026-08-12**, tras el mismo arreglo del arnés de sesión sin interesado; el reset de `appRouter` de T111 la benefició directamente, ya que este archivo también tiene dos `testWidgets`
- [X] T113 [P] Prueba de aislamiento en `integration_test/recording_isolation_test.dart` (validación V7): con dos proyectos poblados, ningún listado de grabaciones cruza datos — **confirmado en dispositivo físico el 2026-08-12**, tras el mismo arreglo del arnés de sesión sin interesado
- [X] T114 [P] Prueba de solo lectura en `integration_test/closed_project_capture_test.dart` (validación V8): con proyecto cerrado, el control de grabar **no aparece en ningún instante**, ni siquiera durante la carga — **confirmado en dispositivo físico el 2026-08-12**, tras el mismo arreglo del arnés de sesión sin interesado (sí lo necesitaba: crea una sesión para después cerrar el proyecto y verificar la pantalla de detalle)
- [X] T115 [P] Documentar en `README.md` los comandos nuevos y el requisito de descargar el modelo antes de transcribir
- [X] T116 Ejecutar las validaciones manuales V1 a V5 y V7 a V9 del [quickstart.md](quickstart.md) en un dispositivo Android físico — **completado en el A059P**: las nueve validaciones pasaron, encontrando y arreglando ocho bugs reales en el camino (ninguno cubierto por los tests automáticos existentes): (1-4) reproductor que no sonaba/no cambiaba de ícono/se reiniciaba solo, y grabación recién iniciada marcada `interrupted` por una carrera en `findInterrupted()` — arreglados en una sesión anterior (commit 098f59e); (5) `LiveMarkList` sin forma de cambiar tipo ni eliminar una marca — cero cobertura de test sobre ese widget; (6) la hoja de recuperación nunca aparecía tras un cierre inesperado porque la propia escritura de `findInterrupted()` retroalimentaba `watchBySession` y pisaba el hallazgo con `null` antes de que la pantalla se construyera con el valor real; (7) al reanudar una interrupción, el cronómetro en pantalla reiniciaba a 00:00 en vez de retomar los segundos ya grabados (`RecoverInterrupted` calculaba la duración real pero la descartaba); (8) `AsyncScaffoldBody` — el único switch exhaustivo sobre `AsyncValue` de toda la app — parpadeaba a un spinner de pantalla completa en cada tick de progreso de descarga, porque decidía por el tipo runtime de `AsyncLoading` sin mirar si ya traía un valor previo; y un noveno hallazgo de UX (mensaje "falta el modelo" mostrado también cuando la sesión simplemente seguía abierta y la transcripción ni se había intentado, ahora un estado `TranscriptNotStarted` separado). Los nueve tests de regresión verificados contra el código viejo (fallan sin el fix, pasan con él)
