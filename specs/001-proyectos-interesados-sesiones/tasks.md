...
- [ ] T001 Verificar el toolchain con `flutter --version`: debe ser Flutter 3.44.9 con Dart 3.12.2 según la constitución v1.1.1. Si no coincide, detenerse y resolver antes de continuar
- [ ] T002 Crear el proyecto Flutter en la raíz del repositorio con `flutter create --project-name up_req --org com.upreq --platforms android,ios .`, conservando `.specify/`, `specs/` y `.claude/`
- [ ] T003 Declarar las dependencias exactas en `pubspec.yaml`: flutter_riverpod 3.4.2, riverpod_annotation 4.0.6, drift 2.34.3, go_router, uuid, clock; y en dev_dependencies riverpod_generator 4.0.8, riverpod_lint 3.1.8, drift_dev, build_runner, flutter_lints, integration_test. **Ninguna dependencia de red** (FR-019) y **sin** `sqlite3_flutter_libs`, que drift ≥ 2.32.0 hace innecesario
- [ ] T004 [P] Configurar `analysis_options.yaml` con `include: package:flutter_lints/flutter.yaml` y la clave `plugins: riverpod_lint: 3.1.8` (mecanismo vigente desde la constitución v1.1.1, no `custom_lint`)
